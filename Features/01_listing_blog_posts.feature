Feature: Listing blog posts
  ...
  In order to find a blog post I am interested in
  ...
  Scenario: Viewing latest blog posts
     ...
     Then I should see "Multistage deployment with capifony" title on the blog post list
      And I should see "Setting up a PHP development environment" title on the blog post list
      ...
      And I should not see "Compiling doctrine in symfony 1.4" title on the blog post list
      And I should not see "About" title on the blog post list

  Scenario: Viewing blog post publishing details
     When I visit the list of blog posts page
     Then I should see that the "Multistage deployment with capifony" blog post was written on "7th May 2011" by "Jakub Zalas"
     Then I should see that the "Setting up a PHP development environment" blog post was written on "5th May 2011" by "Leszek"
     Then I should see that the "Nginx configuration for Symfony projects" blog post was written on "28th April 2011" by "Jakub Zalas"

  Scenario: Viewing blog post excerpts
     When I visit the list of blog posts page
     Then I should see an excerpt of the "Multistage deployment with capifony" blog post on the list
      But I should see a full content of the "Setting up a PHP development environment" blog post on the list
      And I should see an introduction of the "Nginx configuration for Symfony projects" blog post on the list
